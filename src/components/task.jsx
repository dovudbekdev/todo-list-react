export function Task({ id, done, title, setTasks }) {
  const trashIcon = (
    <svg
  ...
  return (
    <>
      <div className={done ? "item item--done" : "item"} id={id}>
        <div
          className="checkbox"
          onClick={() =>
            setTasks((pState) =>
              pState.map((t) => (t.id == id ? { ...t, done: !t.done } : t)),
            )
          }
        ></div>
        <p>{title}</p>
        <button
          onClick={(e) =>
            setTasks((pState) => pState.filter((t) => t.id !== id))
          }
        >
          {trashIcon}
        </button>
